...
class messageModel {
  static async getData(req) {
    try {
      return await Message.find({ delete: { $eq: 0 }, room_id: req.room_id })
        .select()
  ...
  static async getMessage(req) {
    try {
      const findById = await Message.find({ user_id: req.user_id });
      if (findById) {
        const getHistory = await Message.find({ room_id: req.room_id })
          .select()
          .sort({ create_date: "asc" })
          .skip(req.start)
          .limit(req.limit)
    ...
    try {
      ...
      return await Message.aggregate(
        [
          ...
          roomParticipant.findOneAndUpdate({user_id : "", account : 'admin', 'room_id' : req.room_id}, {last_online  : req.now}, (err, result) =>{
